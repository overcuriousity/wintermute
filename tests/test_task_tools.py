...
class TestBuildSchedule:
    ...
    def test_interval_seconds_coerced_to_int(self):
        sched, desc = _build_schedule({"schedule_type": "interval", "interval_seconds": "3600"})
    ...
    def test_day_of_month_must_be_int(self):
        with pytest.raises(ValueError, match="day_of_month must be an integer"):
            _build_schedule({"schedule_type": "monthly", "at": "08:00", "day_of_month": "first"})
...
class TestResolveExecutionMode:
    ...
    def test_cleared_prompt_resolves_to_reminder(self):
        mode, background = _resolve_execution_mode(
            "daily", None, None, True, background_provided=True
        )
    ...
    def test_legacy_inference_notify(self):
        mode, background = _resolve_execution_mode(
            "daily", "do x", None, True, background_provided=True
        )
    ...
    def test_legacy_inference_silent(self):
        mode, background = _resolve_execution_mode(
            "daily", "do x", None, False, background_provided=True
        )
...
class TestTaskUpdate:
    ...
    def test_update_reregisters_job(self, db, deps):
        ...
        res = _result(
            tool_task(
                {"action": "update", "task_id": task_id, "content": "check inbox"},
                thread_id="t1",
                tool_deps=deps,
            )
        )
    ...
    def test_clearing_prompt_converts_to_reminder(self, db, deps):
        ...
        res = _result(
            tool_task(
                {"action": "update", "task_id": task_id, "ai_prompt": ""},
                thread_id="t1",
                tool_deps=deps,
            )
        )
    ...
    def test_scoped_task_not_editable_from_other_thread(self, db, deps):
        ...
        res = _result(
            tool_task(
                {"action": "update", "task_id": task_id, "content": "hijack"},
                thread_id="t2",
                tool_deps=deps,
            )
        )
    ...
    def test_unscoped_task_editable_from_any_thread(self, db, deps):
        ...
        res = _result(
            tool_task(
                {"action": "update", "task_id": task_id, "content": "edited via tool"},
                thread_id="t9",
                tool_deps=deps,
            )
        )
...
class TestTaskLifecycle:
    ...
    def test_delete_survives_remove_job_failure(self, db, deps):
        ...
        res = _result(
            tool_task({"action": "delete", "task_id": task_id}, thread_id="t1", tool_deps=deps)
        )
        assert res["status"] == "ok"
    ...
    def test_delete_missing_task_does_not_touch_scheduler(self, db, deps):
        res = _result(
            tool_task({"action": "delete", "task_id": "nope"}, thread_id="t1", tool_deps=deps)
        )
        assert res["status"] == "not_found"
    ...
    def test_pause_removes_job_after_db_write(self, db, deps):
        task_id = self._add_scheduled(deps)
        res = _result(
            tool_task({"action": "pause", "task_id": task_id}, thread_id="t1", tool_deps=deps)
        )
        assert res["status"] == "ok"
    ...
    def test_resume_recreates_job(self, db, deps):
        ...
        deps.task_scheduler.ensure_job.reset_mock()
        res = _result(
            tool_task({"action": "resume", "task_id": task_id}, thread_id="t1", tool_deps=deps)
        )
        assert res["status"] == "ok"
        deps.task_scheduler.ensure_job.assert_called_once()
    ...
    def test_complete_requires_reason(self, db, deps):
        task_id = self._add_scheduled(deps)
        res = _result(
            tool_task({"action": "complete", "task_id": task_id}, thread_id="t1", tool_deps=deps)
        )
        assert "error" in res
        deps.task_scheduler.remove_job.assert_not_called()
