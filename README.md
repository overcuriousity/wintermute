...
- **Persistent memory** — vector-indexed memory store (FTS5 / local_vector / Qdrant backends) with add-time dedup, tasks in SQLite (active goals / working memory with priorities and optional schedules), and vector-indexed skills survive restarts and are injected into every prompt
