...
| `max_tokens` | no | `4096` | Maximum tokens per response. Set to `0` to remove the limit (`max_output_tokens` is used if configured, otherwise the parameter is omitted and the API/proxy applies its own default). Sub-sessions always call with `0` internally to prevent large tool calls (`write_file`, `execute_shell` heredocs) from being truncated mid-JSON. |
| `max_output_tokens` | no | `0` (not set) | The model's hard output ceiling (e.g. `8192` for DeepSeek). Required when using proxies like LiteLLM that reject `max_tokens` values above the model's actual maximum. When `max_tokens` resolves to `0` (sub-sessions), this value is sent instead of omitting the parameter. |
...
| `proactive_target_thread_id` | no | `"default"` | Deprecated in favour of per-room opt-in via `/proactive on`. Kept for compatibility; has no effect when rooms are opted in. |

> **Experimental — Proactive delivery to rooms:** Proactive sub-sessions can deliver their results to specific Matrix/Signal rooms. Each room must opt in via the `/proactive on` slash command. Results are delivered to all opted-in rooms simultaneously. Disable delivery for a room with `/proactive off`. Because proactive sessions run with full tool access (including the ability to spawn sub-workers), they can take autonomous actions — not just summarise. Users will see the final LLM-processed response in their opted-in room.
